...
def variational_autoencoder(encoding_layer_dim, input_shape, X, X_test):
    ...
    z_mean = Dense(encoding_layer_dim, activation='relu', name='encoded')(h2)
    ...
    def vae_loss_with_intraspread(x, x_decoded_mean):
            xent_loss = input_shape * metrics.binary_crossentropy(x, x_decoded_mean)
            kl_loss = - 0.5 * K.sum(1 + z_log_var - K.square(z_mean) - K.exp(z_log_var), axis=-1)
            W = K.variable(value=vae.get_layer('encoded').get_weights()[0])
            intra_spread_loss = K.mean(K.sqrt(K.sum(K.square(K.mean(W, axis=0) - W), axis=1)), axis=-1)
            return K.mean(xent_loss + kl_loss + intra_spread_loss)
    
        
    vae.compile(optimizer='rmsprop', loss = vae_loss_with_intraspread, metrics=['accuracy'])
...
for feature in features:
    ...
    for act in activities:
        ...
        for us in users:
            filepath = '../../myTrainingData/featuresFilt_' + act + '#' + str(us) + '.csv'
            if os.path.isfile(filepath):
                ...
                np.savetxt("./resultsFusedVariational5AE/AEResult_featuresFilt_" + act + "#" + str(us) +".csv", encoded_fused, delimiter=',')
