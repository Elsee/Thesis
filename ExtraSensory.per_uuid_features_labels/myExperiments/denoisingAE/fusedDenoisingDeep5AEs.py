...
for usr in range(len(allUsersData)):
    us = usr + 1
    totalData = pd.read_csv('../extractedFeatures/featuresFilt_walking#' + str(us) + '.csv')
    ...
    np.savetxt('./resultsFusedDenoising5AE/AEResult_featuresFilt_walking#' + str(us) +".csv", encoded_fused, delimiter=',')
