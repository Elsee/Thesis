...
def add_mask(featureSpace):
    mask = np.random.choice([0, 1], size=(featureSpace.shape), p=[AMOUNT_OF_ZEROS, 1-AMOUNT_OF_ZEROS])
    
    return featureSpace*mask
...
for act in activities:
    for us in range(1,57):
        ...
        if os.path.isfile(filepath): 
            ...
            np.savetxt("./resultsFusedDenoising5AE/" + act + "/AEResult_featuresFilt_" + act + "#" + str(us) +".csv", encoded_fused, delimiter=',')
