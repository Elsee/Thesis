...
for feature in features:
    ...
    for act in activities:
        ...
        for us in range(56):
            ...
            filepath = '../extractedFeatures/' + activityType+ '/' + featuresType +'_' + activityType + '#' + str(userNum) + '.csv'
            
            if os.path.isfile(filepath):
                filenames = glob.glob('../extractedFeatures/' + activityType+ '/' + featuresType +'_' + activityType + '#*.csv')
                ...
                currentUserData = pandas.read_csv(filepath, header = 0)
        ...
        if (len(row_accuracy)):
            with open("./svmAuth/results_new_accuracy/output" + activityType+ ".csv",'a') as f:
                writer = csv.writer(f, dialect='excel')
                writer.writerow(row_accuracy)  
             
            allUsersNum = len(os.listdir('../extractedFeatures/' + activityType))
            
            with open('./svmAuth/' + feature + "_svmResults_" + act + ".txt", "a") as myfile:
                    myfile.write("Mean: \nFRR: " + str("%.5f" % (sumFRR/allUsersNum)) + "\nFAR: " + str("%.5f" % (sumFAR/allUsersNum)) + "\n\n\n")
