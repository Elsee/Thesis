...
def add_mask(featureSpace):
    featureSpace = preprocessing.normalize(np.asarray(featureSpace, dtype= np.float32), norm='l2');
    return random_noise(featureSpace, mode='s&p', amount=0.05)
...
for feature in features:
    ...
    for act in activities:
        ...
        for us in users:
            ...
            x_train_stat_noisy = add_mask(x_train_stat)
            x_test_stat_noisy = add_mask(x_test_stat)
            
            x_train_time, x_test_time = train_test_split(timeData, test_size=0.2)
            x_train_time_noisy = add_mask(x_train_time)
            x_test_time_noisy = add_mask(x_test_time)
            
            x_train_fft, x_test_fft = train_test_split(fftData, test_size=0.2)
            x_train_fft_noisy = add_mask(x_train_fft)
            x_test_fft_noisy = add_mask(x_test_fft)
            
            x_train_wavelet, x_test_wavelet = train_test_split(waveletData, test_size=0.2)
            x_train_wavelet_noisy = add_mask(x_train_wavelet)
            x_test_wavelet_noisy = add_mask(x_test_wavelet)
            ...
            x_train_fused_noisy = add_mask(x_train_fused)
            x_test_fused_noisy = add_mask(x_test_fused)
            ...
            np.savetxt("./resultsFusedDenoisingSandP5AE/AEResult_" + feature + "_" + act + '#' + str(us) +".csv", encoded_fused, delimiter=',')